    ...
    def _configure_chrome_options(self) -> Options:
        ...
        if self.chrome_data_path:
            ...
            if not os.path.exists(self.chrome_data_path):
                logger.warning(
                    f"Chrome user data directory does not exist: {self.chrome_data_path}"
                )
                ...
                try:
                    ...
                    logger.error(
                        f"Failed to create Chrome user data directory: {str(e)}"
                    )
        ...
        if self.driver_path:
            if not os.path.exists(self.driver_path):
                raise FileNotFoundError(
                    f"Chrome driver not found at: {self.driver_path}"
                )
