...
class LinkedInJobManager:
    def __init__(
        self, scraper, job_applier, resume_generator, unit_of_work: MongoUnitOfWork
    ):
    ...
    def save_resume(
        self, resume_content: dict, company_name: str, job_title: str
    ) -> str:
        ...
        # Create job info
        job_info = JobInfo(company_name=company_name, job_title=job_title)
    ...
    async def run_job_search_and_apply(
        self, keywords: str, location: str, num_jobs: int, resumes: list, ai_strategy
    ):
        ...
        for job, resume_content in zip(jobs[:num_jobs], resumes):
            # Save resume
            resume_path = self.save_resume(resume_content, job["company"], job["title"])
    ...
    def search_and_get_job_descriptions(
        self, keywords: str, location: str, num_jobs: int
    ) -> list:
        self.scraper.search_jobs(keywords, location)
        time.sleep(5)
