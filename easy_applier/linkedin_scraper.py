...
class LinkedInScraper:
    def __init__(self, email, password, profile_name=None, headless=False):
        ...
        self.connection = ConnectionFactory.create_selenium_connection(
            profile_name=profile_name, headless=headless
        )
        ...
        for attempt in range(max_retries):
            try:
                ...
                try:
                    verify_device_button = WebDriverWait(self.driver, 5).until(
                        EC.presence_of_element_located(
                            (
                                By.CSS_SELECTOR,
                                "button[data-litms-control-urn='verify_pin_submit']",
                            )
                        )
                    )
                ...
                if attempt < max_retries - 1:
                    logger.warning(
                        f"Login attempt {attempt + 1} failed, retrying... Error: {str(e)}"
                    )
                    time.sleep(2)
                else:
        ...
        for attempt in range(max_retries):
            try:
                ...
                try:
                    ...
                    logger.warning(
                        "Session may have expired, attempting to login again"
                    )
                    self.perform_login()
                ...
                for selector in selectors:
                    ...
                    logger.warning(
                        "Job listings not immediately visible, trying to scroll..."
                    )
                ...
                logger.info(
                    f"Successfully loaded job search for {job_title} in {location}"
                )
                return True
                ...
                if attempt < max_retries - 1:
                    logger.warning(
                        f"Search attempt {attempt + 1} failed, retrying... Error: {str(e)}"
                    )
                    time.sleep(2)
                else:
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    if attempt == max_retries - 1:
                        raise
                    logger.warning(
                        f"Attempt {attempt + 1} to find job cards failed, retrying..."
                    )
                    time.sleep(2)
