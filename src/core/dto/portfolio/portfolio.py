...
class PortfolioDTO:
    ...
    def from_db_models(cls, portfolio: Portfolio, profile: Profile):
        ...
        return cls(
            name=LatexEscaper.escape_text(
                profile.personal_information.get("full_name", "")
            ),
            phone=LatexEscaper.escape_text(
                profile.personal_information.get("phone", "")
            ),
            email=LatexEscaper.escape_text(
                profile.personal_information.get("email", "")
            ),
            linkedin=LatexEscaper.escape_text(
                profile.personal_information.get("linkedin", "")
            ),
            github=LatexEscaper.escape_text(
                profile.personal_information.get("github", "")
            ),
            address=LatexEscaper.escape_text(
                profile.personal_information.get("address", "")
            ),
            website=LatexEscaper.escape_text(
                profile.personal_information.get("website", "")
            ),
