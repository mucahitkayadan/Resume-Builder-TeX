...
class SettingsPage:
    ...
    def _render_llm_settings(self) -> None:
        ...
        with get_unit_of_work() as uow:
            ...
            current_prefs = (
                preferences.get("llm_preferences", {}) if preferences else {}
            )
        ...
        model_type = st.selectbox(
            ...
            index=self.model_selector.model_types.index(
                current_prefs.get("model_type", "Claude")
            ),
        ...
        model_name = st.selectbox(
            ...
            index=(
                self.model_selector.model_options[model_type].index(
                    current_prefs.get("model_name", "claude-3-5-sonnet-20240620")
                )
                if current_prefs.get("model_name")
                in self.model_selector.model_options[model_type]
                else 0
            ),
            ...
            with st.expander("Section Processing"):
                ...
                for section in [
                    ...
                    section_values[section] = st.selectbox(
                        ...
                        index=(
                            0
                            if section_preferences.get(section) == "Process"
                            else (
                                1
                                if section_preferences.get(section) == "Hardcode"
                                else 2
                            )
                        ),
            ...
            if st.button("Save Feature Settings", key="save_features_button"):
                try:
                    uow.users.update_feature_preferences(
                        st.session_state["user_id"],
                        {"check_clearance": clearance_enabled, "auto_save": auto_save},
                    )
